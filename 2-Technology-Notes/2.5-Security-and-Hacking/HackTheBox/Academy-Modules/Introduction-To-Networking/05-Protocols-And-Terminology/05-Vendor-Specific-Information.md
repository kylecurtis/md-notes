...
```bash
user@htb[/htb]$ telnet 10.129.10.2
...
![[8023-Legacy-Ethernet-Frame.webp]]
...
```bash
user@htb[/htb]$ sudo vconfig add eth0 20
...
```bash
user@htb[/htb]$ sudo ip addr add 192.168.1.1/24 dev eth0.20
user@htb[/htb]$ sudo ip link set up eth0.20
```

At last, we can check whether the interface has changed states to up:

```bash
user@htb[/htb]$ ip a | grep eth0.20
...
![[Windows-Device-Manager-Adapter-Properties-VLAN-ID.png]]
...
```bash
user@htb[/htb]$ tshark -r "The Ultimate PCAP v20221220.pcapng" -T fields -e vlan.id | sort -n -u
